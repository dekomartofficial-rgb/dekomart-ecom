...
class User {
        ...
        try {
            ...
            const screenList = result.recordsets[0].map((s) => {
                return {
                    ...
                    Children: result.recordsets[1].filter(
                        (c) => c.GroupName === s.GroupName
                    ),
                };
            });
    ...
    static GetUser = async (req, res) => {
        try {
            ...
            const formattedData = result.recordsets[0].map((row) => ({
                ...row,
                UserId: row.UserId ? BigInt(row.UserId) : 0,
            }));

            res.status(200).json(formattedData[0].UserId + 1n);
        } catch (e) {
            res.status(500).json({ err: "Error Occur:" + e });
        }
    };
